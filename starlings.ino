...
// ==========================================
const char* VERSION  = "1.0"; // <--- Incrémente ce chiffre (1.1, 1.2...) pour déclencher la mise à jour
...
// ==========================================
//  MURMURATION SETTINGS (Tes réglages)
// ==========================================
...
void check_for_updates() {
  ...
  if (httpCode == 200) {
    ...
    if (newVersion != VERSION) {
      ...
      // L'ESP32 va télécharger le .bin et redémarrer tout seul
...
// ==========================================
//  TES FONCTIONS DE PHYSIQUE (Inchangées)
// ==========================================
...
static void limitSpeed(Bird &b){
  ...
  float s = 1.0f / inv;
  if (s > MAX_SPEED){ b.vx *= MAX_SPEED*inv; b.vy *= MAX_SPEED*inv; }
  else if (s < MIN_SPEED){ b.vx *= MIN_SPEED*inv; b.vy *= MIN_SPEED*inv; }
}
...
static void updateBird(int i){
  ...
  for(int j=0;j<NUM_BIRDS;j++){
    ...
    if (d2 < (NEIGHBOR_RAD*NEIGHBOR_RAD) && d2 > 1e-4f){
      ...
      if (d < SEP_DIST){
        float w = pow((SEP_DIST - d) / SEP_DIST, 2);
        sx -= dx * invd * w; sy -= dy * invd * w;
      }
      tx += o.turnSignal * (-dy) * invd; ty += o.turnSignal * ( dx) * invd;
      n++;
    }
  }
  ...
  if (n>0){
    float invn = 1.0f/n; ax *= invn; ay *= invn; cx *= invn; cy *= invn;
    b.vx += (ax - b.vx)*alignF; b.vy += (ay - b.vy)*alignF;
    b.vx += (cx - b.x)*COHESION_BASE; b.vy += (cy - b.y)*COHESION_BASE;
    b.vx += sx * SEPARATION_BASE; b.vy += sy * SEPARATION_BASE;
    b.vx += tx * 0.010f; b.vy += ty * 0.010f;
  }
  b.vx += fx * FLOW_STRENGTH + driftX * DRIFT_STRENGTH;
  b.vy += fy * FLOW_STRENGTH + driftY * DRIFT_STRENGTH;
  if (gustOn){ float rvx = -b.vy, rvy = b.vx; b.vx += rvx * GUST_TURN; b.vy += rvy * GUST_TURN; }
  applyBorders(b); limitSpeed(b);
  b.turnSignal = b.turnSignal * 0.88f + clampf(fabsf(fx*(-b.vy)+fy*b.vx)*0.35f + (gustOn?0.18f:0.0f),0,1)*0.12f;
...
void setup() {
  ...
  // Premier check update au démarrage
  check_for_updates();
}
...
void loop() {
  ...
  // Vérification de mise à jour toutes les 30 min sans bloquer l'anim
